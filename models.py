...
class PEMformer(VisionTransformer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.head  = nn.Sequential(
                    nn.Conv2d(self.embed_dim, 16, kernel_size=3, stride=1, padding=1),
                    nn.BatchNorm2d(16),nn.ReLU(),
                    nn.Conv2d(16,self.num_classes,kernel_size=3,stride=1,padding=1),
                    nn.BatchNorm2d(self.num_classes),nn.ReLU())
    ...
    def forward_features(self, x, erase,n=12,):
        # x = self.conv(x)
        B, nc, w, h = x.shape
    ...
    def forward(self, x, erase_weight, attention_type='fused',):
        w, h = x.shape[2:]
        x_cls, patch_tokens, attn_weights = self.forward_features(x,erase=erase_weight)
        x_patch = patch_tokens.clone()
        n, p, c = x_patch.shape
        if w != h:
            w0 = w // self.patch_embed.patch_size[0]
            h0 = h // self.patch_embed.patch_size[0]
        ...
        x_conv = self.head(x_patch)
        x_patch_logits = self.avgpool(x_conv).squeeze(3).squeeze(2)

        attn_weights = torch.stack(attn_weights)  # 12 * B * H * N * N
        attn_weights = torch.mean(attn_weights, dim=2)  # 12 * B * N * N

        feature_map = x_conv.detach().clone()  # B * C * 14 * 14
        feature_map = F.relu(feature_map)

        n, c, h, w = feature_map.shape
        mtatt = attn_weights[:, 0,0:self.num_classes, self.num_classes:][1].reshape([1, 1, h, w])  # 多个层求和
        ...
        return x_cls_logits, x_patch_logits, cams, patch_attn, mtatt, patch_tokens
...
class convformer(nn.Module):
    ...
    def forward(self, method, x,erase):
        B = x.shape[0]
        x = x*erase
        cls_tokens = self.cls_token.expand(B, -1, -1)
        ...
        n, c, h, w = feature_map.shape
        attn_weights_no_cls = attn_weights.sum(0)[:, 1:, 1:]  # 1 784,784
        conv_flat = x_conv.flatten(start_dim=2)  # 1,1 784
        ...
        if method == 'transcam':
            cams = torch.einsum('nhw,ncw->nch', attn_weights_no_cls, conv_flat).reshape([n, self.num_classes, h, w])

        if method == 'attnagg':
            cams = torch.einsum('nhw,ncw->nch', attn_weights_no_cls.transpose(1, 2), conv_flat).reshape([n, self.num_classes, h, w])

        return conv_logits, trans_logits, x_conv, attn_weights[:,:,:,:].sum(0)[:, 0, 1:].reshape([n, h, w]).unsqueeze(1), attn_weights[:,:, 1:, 1:], x_t


@register_model
def Pem_former(pretrained=False, **kwargs):
    model = PEMformer(
        patch_size=16, embed_dim=192, depth=8, num_heads=6, in_chans=1, mlp_ratio=4, qkv_bias=True,
...
@register_model
def Conv_former(pretrained=False, **kwargs):
    model = convformer(
        patch_size=16, channel_ratio=1, embed_dim=192, depth=8,in_chans=1,
                      num_heads=6, mlp_ratio=4, qkv_bias=True, drop_rate=0.0, drop_path_rate=0.1, num_classes=1,**kwargs)
    return model
